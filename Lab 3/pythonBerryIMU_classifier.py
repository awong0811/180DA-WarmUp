...
while True:
    ...
    # Normalize accelerometer readings to g (assuming 1g = 9.81 m/s^2)
    ACCx_g = ACCx / 9.81
    ACCy_g = ACCy / 9.81
    ACCz_g = ACCz / 9.81
    ACCz_g += 415 # add to remove bias

    # Define thresholds for motion detection
    FORWARD_THRESHOLD = 40     # g-units, adjust experimentally
    UPWARD_THRESHOLD = 40      # g-units, adjust experimentally
    STILL_THRESHOLD = 10       # below this means "N/A"

    # Initialize movement label
    movement = "N/A"

    # Classify motion
    if abs(ACCx_g) > FORWARD_THRESHOLD and abs(ACCx_g) > abs(ACCz_g):
        movement = "Forward Push"
    elif abs(ACCz_g) > UPWARD_THRESHOLD and abs(ACCz_g) > abs(ACCx_g):
        movement = "Upward Lift"
    elif abs(ACCx_g) < STILL_THRESHOLD and abs(ACCz_g) < STILL_THRESHOLD:
        movement = "N/A"

    # Append movement classification to output
    outputString += f"\t# Movement: {movement} #"
    print(outputString)
    print(f"acc_z: {ACCz_g}")
    print(f"acc_x: {ACCx_g}")
